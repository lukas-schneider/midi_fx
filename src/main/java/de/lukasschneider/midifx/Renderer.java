...
class Renderer implements GLEventListener {
  ...
  public static final long NANOSECONDS_PER_FRAME
      = (1000 * 1000 * 1000) / FRAMES_PER_SECOND;

  public static final int START_TIME_S = -5;
  public static final int START_FRAME = START_TIME_S * FRAMES_PER_SECOND;
  ...
  private boolean recorderNeedsClose = false;
  private boolean idle = false;

  private int displayWidth = (int) WIDTH, displayHeight = (int) HEIGHT;
  ...
  public void display(GLAutoDrawable glAutoDrawable) {
    ...
    if (recording) {
      ...
      recorder.addFrame(
          glAutoDrawable,
          frame - START_FRAME,
          nanoTime - (START_FRAME * NANOSECONDS_PER_FRAME)
      );
      drawRec(gl);

      frame++;
    } else if(recorderNeedsClose) {
      recorderNeedsClose = false;
      recorder.close();
    }
  }
  ...
  private void drawCircle(GL2 gl, double x, double y, double radius) {
    ...
    for (int i = 0; i < 360; i++) {
      ...
      gl.glVertex2d(
          x + Math.cos(degInRad) * radius,
          y + Math.sin(degInRad) * radius
      );
    }
  ...
  void record() {
    ...
    try {
      recorder = new Recorder(displayWidth, displayHeight);
  ...
  void stop() {
    idle = true;
    if(recording) recorderNeedsClose = true;
    recording = false;
    playing = false;
  }
}
